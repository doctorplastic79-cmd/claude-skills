...
- Run:
```bash
node "$(git rev-parse --show-toplevel)/.claude/codex-plugin/scripts/codex-companion.mjs" adversarial-review "$ARGUMENTS"
```
...
```typescript
Bash({
  command: `node "$(git rev-parse --show-toplevel)/.claude/codex-plugin/scripts/codex-companion.mjs" adversarial-review "$ARGUMENTS"`,
